 ...
 * @arg     1:          失败
 */
uint8_t atk_qmi8658_write_nbytes(uint8_t reg, uint8_t* data, uint8_t len)
{
    uint8_t i;
...
{
    return atk_qmi8658_read_byte(Register_StatusInt);
}
...
{
    return atk_qmi8658_read_byte(Register_Status0);
}
...
{
    return atk_qmi8658_read_byte(Register_Status1);
}
 ...
 * @arg     1: 失败
 */
int atk_qmi8658_send_ctl9cmd(enum Ctrl9Command cmd)
{
    ...
    while(retry++ < 3)
    {
        ...
        atk_qmi8658_write_byte(Register_Ctrl9, Ctrl9_Cmd_Ack);          /* write commond  0x00 to ctrl */
        count = 0;
        atk_qmi8568_read_nbytes(status_reg, &status1, 1);
 ...
 * @retval  无
 */
void atk_qmi8658_enable_ahb_clock(int enable)
{
    if (enable)
    {
        atk_qmi8658_write_byte(Register_Ctrl7, 0x00);
        atk_qmi8658_write_byte(Register_Cal1_L, 0x00);
        atk_qmi8658_send_ctl9cmd(Ctrl9_Cmd_AHBClockGating);
    }
    else
    {
        atk_qmi8658_write_byte(Register_Ctrl7, 0x00);
        atk_qmi8658_write_byte(Register_Cal1_L, 0x01);
        atk_qmi8658_send_ctl9cmd(Ctrl9_Cmd_AHBClockGating);
    }
}
...
uint8_t atk_qmi8658_init(void)
{
   ...
   // printf("校准成功！！\r\n");
    ...
    atk_qmi8658_enablesensors(3);     /* 使能陀螺仪、加速度计 */
